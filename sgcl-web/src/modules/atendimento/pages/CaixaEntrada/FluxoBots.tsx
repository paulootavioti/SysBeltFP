    ...
    {fluxo && <>
      <section className="fluxo-canais" aria-label="Conexões dos bots">
        {canais.map((canal) => <article className="fluxo-canal" key={canal.id}>
          ...
          <Situacao degrau={({ CONECTADO: "neutro", ERRO: "acao", VALIDANDO: "atencao", INATIVO: "inativo" } as Record<string, DegrauSituacao>)[canal.status] ?? "inativo"}>{canal.status === "CONECTADO" ? <LuCheck /> : canal.status === "ERRO" ? <LuTriangleAlert /> : <LuRefreshCw />} {canal.status === "CONECTADO" ? "Conectado" : canal.status === "ERRO" ? "Falha de conexão" : canal.status === "VALIDANDO" ? "Validação pendente" : "Inativo"}</Situacao>
