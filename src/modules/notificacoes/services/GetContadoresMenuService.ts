...
// Próximas Promoções), então o cálculo reaproveita a mesma regra de
// negócio que essas telas já usam, só trocando a lista completa por uma
// contagem. Pensado pra ser barato o bastante pra rodar em toda troca de
// página (ver useContadoresMenu no frontend).
export class GetContadoresMenuService {
  async execute(unidadeId: number | null): Promise<ContadoresMenu> {
    const [mensalidadesVencidas, contratosAguardandoAssinatura, proximasGraduacoes] = await Promise.all([
