...
export interface FonteContagem {
  listarUnidades(tenantKey?: string): Promise<UnidadeAgregada[]>;
  contarAlunosAtivos(tenantKey?: string): Promise<ContagemAgregada[]>;
}

const fontePrisma: FonteContagem = {
  listarUnidades: (tenantKey) => {
    const prisma = prismaDaRequisicao();
    return prisma.unidade.findMany({
      where: tenantKey ? { conta: { tenantKey } } : undefined,
      select: { id: true, nome: true, ativo: true }, orderBy: { id: "asc" },
    });
  },
  contarAlunosAtivos: async (tenantKey) => {
    const prisma = prismaDaRequisicao();
    const contagens = await prisma.alunoUnidade.groupBy({
      by: ["unidadeId"],
      where: { aluno: { ativo: true }, unidade: { ativo: true, ...(tenantKey ? { conta: { tenantKey } } : {}) } },
      _count: { _all: true },
    });
...
export class GerarSnapshotContagemService {
  ...
  async execute(tenantKey: string, agora = new Date()): Promise<SnapshotContagemV1> {
    const [unidades, contagens] = await Promise.all([
      this.fonte.listarUnidades(tenantKey), this.fonte.contarAlunosAtivos(tenantKey),
    ]);
