...
const alvosSchema = z.object({ usuarioIds: z.array(z.number().int().positive()).min(1) });
...
export class RedefinirSenhaService {
  async execute(token: string, senha: string) {
    ...
    const registro = await prisma.tokenRedefinicaoSenha.findUnique({
      where: { tokenHash: hashTokenRedefinicaoSenha(token) },
    });

    if (!registro || registro.tipo !== "EQUIPE" || registro.usadoEm || registro.expiraEm <= new Date()) {
    ...
    await prisma.$transaction([
      prisma.usuario.updateMany({ where: { id: { in: alvos.data.usuarioIds }, ativo: true }, data: { senha: senhaHash } }),
